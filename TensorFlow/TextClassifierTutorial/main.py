...
def load_dataset(directory):
    pos_df = load_directory_data(os.path.join(directory, "obl"))
    neg_df = load_directory_data(os.path.join(directory, "rec"))
    pos_df["polarity"] = 1
    neg_df["polarity"] = 0
...
def download_and_load_datasets(force_download=False):
    ...
    train_df = load_dataset(os.path.join(mypath, "data", "train"))
    test_df = load_dataset(os.path.join(mypath, "data", "test"))
...
predict_train_input_fn = tf.estimator.inputs.pandas_input_fn(train_df,
                                                             train_df["polarity"],
                                                             shuffle=False)
